...
const TimeTracker: React.FC<TimeTrackerProps> = ({ workDays, onUpdateWorkDays }) => {
  const [currentTime, setCurrentTime] = useState(new Date());
  
  const today = new Date();
  const todayWorkDay = workDays.find(day => isSameDay(day.date, today));
  
  const isWorkStarted = todayWorkDay?.workTime.start && !todayWorkDay?.workTime.end;
  const isLunchStarted = todayWorkDay?.lunchBreak?.start && !todayWorkDay?.lunchBreak?.end;
  ...
  const stopWorkDay = () => {
    ...
    let totalWorkHours = (workEnd.getTime() - workStart.getTime()) / (1000 * 60 * 60);
    
    // Subtract lunch break time if exists
    if (lunchBreak?.start && lunchBreak?.end) {
      const lunchDuration = (lunchBreak.end.getTime() - lunchBreak.start.getTime()) / (1000 * 60 * 60);
  ...
  const getWorkDuration = () => {
    ...
    if (todayWorkDay.lunchBreak?.start) {
      ...
      const lunchEnd = todayWorkDay.lunchBreak.end || (isLunchStarted ? currentTime : lunchStart);
      ...
      <CardContent className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center">
            ...
            <Badge variant={isWorkCompleted ? 'default' : isWorkStarted ? 'secondary' : 'outline'} className="mt-1">
              {getCurrentStatus()}
            </Badge>
          </div>
          <div className="text-center">
            ...
            <CardContent className="space-y-2">
              {todayWorkDay.workTime.start && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatTime(todayWorkDay.workTime.start)}</span>
                </div>
              )}
              {todayWorkDay.workTime.end && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Fin:</span>
                  <span className="font-medium">{formatTime(todayWorkDay.workTime.end)}</span>
                </div>
              )}
              {todayWorkDay.lunchBreak?.start && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {formatTime(todayWorkDay.lunchBreak.start)} - {todayWorkDay.lunchBreak.end ? formatTime(todayWorkDay.lunchBreak.end) : 'En cours'}
                  </span>
                </div>
              )}
              {todayWorkDay.isComplete && (
                <div className="flex justify-between border-t pt-2">
                  ...
                  <span className="font-bold text-primary">{todayWorkDay.totalWorkHours}h</span>
                </div>
              )}
            </CardContent>
          </Card>
